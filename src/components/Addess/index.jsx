...
import {
  saveDptCustomAddress,
  saveDptDropAddress,
  saveReturnCustomAddress,
...
const Address = (Props) => {
  const { tAddress, type, handleShow, returnBooking } = Props;
        ...
        <div className="pt-2 pb-3 border-b border-[#ddd] w-full">
          ...
          >
            <div className="grid grid-cols-2 gap-2.5">
              <small>
                {tAddress("address")} <span className="text-[#DF2029]">*</span>
              </small>
              <div className="col-span-full">
                <input
                  type="text"
                  placeholder={tAddress("please_enter_address")}
                  name="address"
              ...
              <div className="col-span-1">
                <select
                  className="bg-white w-full border border-[#d9d9d9] h-full px-2.5 py-2.5"
                  name="province_id"
                  value={formVals.province_id}
                  onChange={(e) => {
                    dispatch(getDistricts(e.target.value));
                    handleChange(e);
                  }}
                >
                  <option>{tAddress("select_province_city")}</option>
                  {provinces.map((p) => (
              ...
              <div className="col-span-1">
                <select
                  className="bg-white w-full border border-[#d9d9d9] h-full px-2.5 py-2.5"
                  name="district_id"
                  value={formVals.district_id}
                  onChange={(e) => {
                    dispatch(getWards(e.target.value));
                    handleChange(e);
                  }}
                >
                  <option>{tAddress("select_district")}</option>
                  {districts.map((d) => (
              ...
              <div className="col-span-1">
                <small>
                  {tAddress("ward")} <span className="text-[#DF2029]">*</span>
                </small>
              </div>
              ...
              <div className="col-span-1">
                <select
                  className="bg-white w-full border border-[#d9d9d9] h-full px-2.5 py-2.5"
                  name="ward_id"
                  value={formVals.ward_id}
                  onChange={(e) => {
                    handleChange(e);
                  }}
                >
                  <option>{tAddress("select_ward")}</option>
                  {wards.map((w) => (
