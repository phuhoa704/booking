        ...
        <div className="w-9/12 py-8 m-auto">
          <div className="grid grid-cols-5 gap-4">
            <div className="col-span-full xl:col-span-2">
              <p className="font-semibold text-xl my-1.5">
                {tFooter("routes")}
              </p>
              <div className="grid grid-cols-1 gap-2.5">
                {routes.map((fsi) => (
                  <div
                    className="col-span-full font-semibold text-sm cursor-pointer"
                    key={fsi.id}
                    onClick={() => {
                  ...
                  >
                    {tFooter("routes")} {localizedFields(fsi).name}
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 gap-2.5">
                {listNews.map((fsi) => (
                  <div
                    className="col-span-full font-semibold text-sm cursor-pointer"
                    key={fsi.id}
                    onClick={() => navigate(`/news/${fsi.slug}`)}
                  >
                    {localizedFields(fsi).name}
                  </div>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-4 gap-4 col-span-full">
              {pages.map((p) => (
                <div className="col-span-full xl:col-span-1" key={p.id}>
                  <p className="font-semibold text-xl my-1.5">
                    {localizedFields(p).name}
                  </p>
                  <div className="grid grid-cols-1 gap-2.5">
                    {p.pages.map((fsi) => (
                      <div
                        className="col-span-full font-semibold text-sm cursor-pointer"
                        onClick={() => navigate(`/pages/${fsi.slug}`)}
                        key={fsi.id}
                      >
                        {localizedFields(fsi).title}
                      </div>
                    ))}
                  </div>
                </div>
