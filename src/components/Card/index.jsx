...
import { saveReturnDropAddress, saveUseDptCustomAddress, saveUseDptDropAddress, saveUseReturnCustomAddress, saveUseReturnDropAddress } from "../../redux/slices/Address";
...
const Card = (Props) => {
    ...
    const { departureCustomAddress, returnCustomAddress, departureDropAddress, returnDropAddress } = useSelector(state => state.address);
    ...
    return (
        <>
            <Modal
                ...
                outlet={<Address type={typeAddress} handleShow={setModalAddress} returnBooking={returnBooking} />}
            />
                    ...
                    <div className="w-full xl:w-largeCardContainer">
                        <div className="flex justify-between mb-1.5">
                            <div className="flex items-center gap-2.5">
                                ...
                                <span className="xl:flex items-center text-sm px-1 rounded hidden" style={{ backgroundColor: background }}>
                                    <FaStar className="mr-2" style={{ color: text }} />
                                    <span className="" style={{ color: text }}>{parseInt(avg)} ({total})</span>
                                </span>
                            </div>
                            <span className="font-bold text-lg" style={{ color: background }}>Từ {formatChangeNumber(data.price.toString())}đ</span>
                        </div>
                        <div className="flex justify-between mb-1.5">
                    ...
                    <div className="my-3">
                        <div className="py-2.5 border-t border-b border-[#d9d9d9]">
                            <ol className="flex items-center w-full text-sm font-medium text-center text-gray-500 dark:text-gray-400 sm:text-base">
                                <li className={`flex md:w-full items-center ${(currentStep === 1) && 'dark:text-blue-500'} sm:after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-4 xl:after:mx-4 dark:after:border-gray-700`}>
                                ...
                                <li className={`flex items-center ${(currentStep === 2) && 'dark:text-blue-500'}`}>
                                    {(currentStep === 2) && (
                                        <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 me-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z" />
                                        </svg>
                        ...
                        {(currentStep === 1) && (
                            <>
                                ...
                                <div className="flex w-full justify-end items-center my-2">
                                    <div className="text-sm mr-2.5">Tổng cộng: <span className="font-semibold" style={{ color: background }}>{formatChangeNumber(subtotal.toString())}đ</span></div>
                                    <button className="p-2 rounded text-white text-sm" style={{ backgroundColor: background, color: text }} onClick={() => setCurrentStep(2)}>Tiếp tục</button>
                                </div>
                            </>
                        )}
                        {(currentStep === 2) && (
                            <>
                                            ...
                                            {data.pickups.map(dp => (
                                                <div class="inline-flex items-center">
                                                    <label class="relative flex items-center p-3 rounded-full cursor-pointer" htmlFor="pickup">
                                                        <input name="pickup" checked={pickup === dp.id} value={dp.id} onChange={() => {
                                                            setPickup(dp.id);
                                                            if (returnBooking) {
                                                    ...
                                                    <label class="mt-px cursor-pointer select-none text-sm flex flex-col" htmlFor="html">
                                                        <span className="" style={{ color: background }}>{moment(new Date(dp.time)).format('HH:mm')} &nbsp;{dp.name}</span>
                                                        <span className="text-xs"><i className="fa-solid fa-location-dot"></i>{dp.address}</span>
                                                    </label>
                                                </div>
                                            ))}
                                            {returnBooking ? ((Object.keys(returnCustomAddress).length > 0) && (
                                                <div class="inline-flex items-center">
                                                    <label class="relative flex items-center p-3 rounded-full cursor-pointer" htmlFor="pickup">
                                                        <input name="pickup" checked={pickup === returnCustomAddress.id} value={returnCustomAddress.id} onChange={() => {
                                                    ...
                                                    <label class="mt-px cursor-pointer select-none text-sm flex flex-col" htmlFor="html">
                                                        <span className="" style={{ color: background }}>{moment(new Date(data.start_time)).format('HH:mm')}</span>
                                                        <span className="text-xs"><i className="fa-solid fa-location-dot"></i>{returnCustomAddress.address}</span>
                                                    </label>
                                                </div>
                                            )) : ((Object.keys(departureCustomAddress).length > 0) && (
                                                <div class="inline-flex items-center">
                                                    <label class="relative flex items-center p-3 rounded-full cursor-pointer" htmlFor="pickup">
                                                        <input name="pickup" checked={pickup === departureCustomAddress.id} value={departureCustomAddress.id} onChange={() => setPickup(departureCustomAddress.id)} type="radio"
                                                            class="before:content[''] peer relative h-5 w-5 cursor-pointer appearance-none rounded-full border border-blue-gray-200 text-gray-900 transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-gray-900 checked:before:bg-gray-900 hover:before:opacity-10"
                                                            id="pickup" />
                                                        <span
                                                            class="absolute text-gray-900 transition-opacity opacity-0 pointer-events-none top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 peer-checked:opacity-100">
                                                    ...
                                                    <label class="mt-px cursor-pointer select-none text-sm flex flex-col" htmlFor="html">
                                                        <span className="" style={{ color: background }}>{moment(new Date(data.start_time)).format('HH:mm')}</span>
                                                        <span className="text-xs"><i className="fa-solid fa-location-dot"></i>{departureCustomAddress.address}</span>
                                                    </label>
                                                </div>
                                            ))}
                                        </div>
                                            ...
                                            {data.drops.map(dd => (
                                                <div class="inline-flex items-center">
                                                    <label class="relative flex items-center p-3 rounded-full cursor-pointer" htmlFor="drop">
                                                        <input name="drop" checked={drop === dd.id} onChange={() => {
                                                            setDrop(dd.id);
                                                            if (returnBooking) {
                                                    ...
                                                    <label class="mt-px cursor-pointer select-none text-sm flex flex-col" htmlFor="html">
                                                        <span className="" style={{ color: background }}>{moment(new Date(dd.time)).format('HH:mm')} &nbsp;{dd.name}</span>
                                                        <span className="text-xs"><i className="fa-solid fa-location-dot"></i>{dd.address}</span>
                                                    </label>
                                                </div>
                                            ))}
                                            {returnBooking ? ((Object.keys(returnDropAddress).length > 0) && (
                                                <div class="inline-flex items-center">
                                                    <label class="relative flex items-center p-3 rounded-full cursor-pointer" htmlFor="drop">
                                                        <input name="drop" checked={drop === returnDropAddress.id} value={returnDropAddress.id} onChange={() => {
                                                    ...
                                                    <label class="mt-px cursor-pointer select-none text-sm flex flex-col" htmlFor="html">
                                                        <span className="" style={{ color: background }}>{moment(new Date(data.end_time)).format('HH:mm')}</span>
                                                        <span className="text-xs"><i className="fa-solid fa-location-dot"></i>{returnDropAddress.address}</span>
                                                    </label>
                                                </div>
                                            )) : ((Object.keys(departureDropAddress).length > 0) && (
                                                <div class="inline-flex items-center">
                                                    <label class="relative flex items-center p-3 rounded-full cursor-pointer" htmlFor="drop">
                                                        <input name="drop" checked={drop === departureDropAddress.id} value={departureDropAddress.id} onChange={() => setDrop(departureDropAddress.id)} type="radio"
                                                            class="before:content[''] peer relative h-5 w-5 cursor-pointer appearance-none rounded-full border border-blue-gray-200 text-gray-900 transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-gray-900 checked:before:bg-gray-900 hover:before:opacity-10"
                                                            id="drop" />
                                                        <span
                                                            class="absolute text-gray-900 transition-opacity opacity-0 pointer-events-none top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 peer-checked:opacity-100">
                                                    ...
                                                    <label class="mt-px cursor-pointer select-none text-sm flex flex-col" htmlFor="html">
                                                        <span className="" style={{ color: background }}>{moment(new Date(data.end_time)).format('HH:mm')}</span>
                                                        <span className="text-xs"><i className="fa-solid fa-location-dot"></i>{departureDropAddress.address}</span>
                                                    </label>
                                                </div>
                                            ))}
                                        </div>
                                ...
                                <div className="flex w-full justify-between items-center my-2">
                                    ...
                                    <div className="flex items-center">
                                        <div className="text-sm mr-2.5">Tổng cộng: <span className="font-semibold" style={{ color: background }}>{formatChangeNumber(subtotal.toString())}đ</span></div>
                                        <button className="p-2 rounded text-white text-sm" style={{ backgroundColor: background }} onClick={() => {
                                            if (returnBooking) {
