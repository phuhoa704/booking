...
const Search = () => {
  ...
  const sortOpts = [
    { id: 1, label: tSearch("default"), value: null },
    { id: 2, label: tSearch("earliest_departure"), value: "asc_time" },
    { id: 3, label: tSearch("latest_departure"), value: "desc_time" },
    { id: 4, label: tSearch("price_ascending"), value: "asc_price" },
    { id: 5, label: tSearch("price_descending"), value: "desc_price" },
  ];
  ...
  useEffect(() => {
    if (state && Object.keys(state).length > 0) {
      ...
      if (state.departure_province_id) {
        let finder = provinces.find(
          (p) => p.id === state.departure_province_id
        );
        if (finder) {
          setFirstPlace({
            label: finder.name,
  ...
  return (
    <>
      <Modal
        ...
        outlet={<ResponsiveFilter tResponsiveFilter={tSearch} />}
      />
      <Modal
        handleShow={setModalSort}
        showStatus={modalSort}
        outlet={<ResponsiveSort tResponsiveSort={tSearch} />}
      />
      <Modal
        handleShow={setModalTime}
        showStatus={modalTime}
        outlet={<ResponsiveTime tResponsiveTime={tSearch} />}
      />
      <Modal
        handleShow={setModalStation}
        showStatus={modalStation}
        outlet={<ResponsiveStation tResponsiveStation={tSearch} />}
      />
      <Modal
        handleShow={setModalSearch}
        showStatus={modalSearch}
        outlet={<ResponsiveSearch tResponsiveSearch={tSearch} />}
      />
          ...
          <div className="w-full hidden xl:block bg-white rounded-lg shadow-secondary mb-4 border border-[#e0e0e0]">
            <div className="flex justify-center py-3">
              <div className="flex pt-1.5 md:pt-0 xl:pt-3">
                ...
                >
                  <i className="fa-solid fa-bus"></i> {tSearch("bus")}
                </div>
              </div>
            </div>
            <div className="grid grid-cols-5 gap-2.5 w-full relative border-t border-[#ddd] py-3.5 px-3 dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                ...
                <div className="grid grid-cols-4 h-full">
                  ...
                  <div className="col-span-2 xl:col-span-1 border-t border-r xl:border-t-0 border-[#d9d9d9] ">
                    <DatePicker
                      ...
                      placeholderText={tSearch("departure_date")}
                      selected={startDate}
                  ...
                  <div className="col-span-2 xl:col-span-1 border-t border-[#d9d9d9] xl:border-none">
                    <DatePicker
                      className="px-2.5 py-2.5 bg-white border border-[#d9d9d9] w-full h-full rounded border-none focus:border-none"
                      placeholderText={tSearch("return_date")}
                      selected={endDate}
              ...
              <div className="col-span-full xl:col-span-1">
                ...
                >
                  <span className="font-semibold">{tSearch("search")}</span>
                </button>
              </div>
            </div>
          </div>
          <div className="w-full grid grid-cols-7 gap-4">
            <div className="col-span-full xl:col-span-2 hidden xl:block">
              <div className="w-full bg-white p-2.5 border border-[#e0e0e0] rounded-lg mb-4">
                <p className="font-semibold mb-2.5">{tSearch("sort")}</p>
                <ul>
                  {sortOpts.map((s) => (
                    <li key={s.id}>
            ...
            <div className="col-span-full xl:col-span-5">
                ...
                searchResult
                  ...
                  .map((sr) => (
                    <Card
                      tCard={tSearch}
                      data={sr}
                      returnBooking={state.returnBooking}
                    />
                  ))}
              <div className="w-full text-center">
                <button className="bg-primary py-2.5 px-2 text-white text-sm rounded">
                  {tSearch("view_more")}
                </button>
              </div>
              <div className="fixed w-full bottom-10 block xl:hidden">
                <div className="rounded-xl bg-[#0D2E58] grid grid-cols-4 divide-x p-2.5 text-white text-sm w-10/12 m-auto">
                  <div
                    className="col-span-1 flex items-center justify-center gap-2 cursor-pointer"
                  ...
                  >
                    <FaRegClock />
                    <span> {tSearch("departure_time")}</span>
                  </div>
                  <div
                    className="col-span-1 flex items-center justify-center gap-2 cursor-pointer"
                    onClick={() => setModalStation(true)}
                  >
                    <FaBus />
                    <span>{tSearch("operator")}</span>
                  </div>
                </div>
              </div>
            </div>
