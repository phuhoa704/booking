...
const MyTicket = () => {
  const { t: tMyTicket } = useTranslation("user_info");
  ...
  const { orderList, orderCompleted, orderCanceled } = useSelector(
    (state) => state.order
  );
  return (
    <>
      ...
      />
      <div>
        <Breadscrum site={tMyTicket("my_ticket")} />
            ...
            <div className="items-end bg-white w-full border border-[#d9d9d9] grid grid-cols-3 rounded">
              {tableList?.map((t) => (
                <div
                  ...
                  className={`text-center py-3 text-sm cursor-pointer hover:text-primary ${
                    openTab === t.id
                      ? "text-primary border-b-2 border-primary font-semibold"
                      : ""
                  }`}
                >
                  <button>{tMyTicket(t.title)}</button>
                </div>
              ))}
            </div>
            {openTab === 1 && (
                  ...
                  <table className="w-full max-h-full table-auto">
                    <thead>
                      <tr className="bg-primary text-sm text-left">
                        <th className="p-2 font-medium text-white xl:p-2 text-center">
                          STT
                        </th>
                        <th className="p-2 font-medium text-white">
                          {tMyTicket("code")}
                        </th>
                        <th className="p-2 font-medium text-white">
                          {tMyTicket("full_name")}
                    ...
                    <tbody className="max-h-full overflow-auto">
                      {orderList.map((o, idx) => (
                        <tr className="text-sm">
                          <td className="border-b border-[#eee] p-2 text-center">
                            {idx + 1}
                          </td>
                          ...
                          <td className={`border-b border-[#eee] p-2`}>
                            <span
                              className={`inline-flex rounded-full p-1 text-white ${
                                o.status === 0 && "bg-pending"
                              } ${o.status === 1 && "bg-approved"} ${
                                o.status === 2 && "bg-completed"
                              } ${o.status === 3 && "bg-canceled"}`}
                            >
                              {STATUS.find((s) => s.value === o.status)
                          ...
                          <td className="border-b border-[#eee] p-2">
                            <button
                              className="bg-primary text-white text-center p-2 rounded"
                              onClick={() => {
                    ...
                    <tbody className="max-h-full overflow-auto">
                      {orderCompleted.map((o, idx) => (
                        <tr className="text-sm">
                          ...
                          <td className={`border-b border-[#eee] p-2`}>
                            <span
                              className={`inline-flex rounded-full p-1 text-white bg-completed`}
                            >
                              {STATUS.find((s) => s.value === o.status)
                                ? STATUS.find((s) => s.value === o.status).label
                                : tMyTicket("updating")}
                    ...
                    <tbody className="max-h-full overflow-auto">
                      {orderCanceled.map((o, idx) => (
                        <tr className="text-sm">
                          ...
                          <td className={`border-b border-[#eee] p-2`}>
                            <span
                              className={`inline-flex rounded-full p-1 text-white bg-canceled`}
                            >
                              {STATUS.find((s) => s.value === o.status)
                                ? STATUS.find((s) => s.value === o.status).label
                                : tMyTicket("updating")}
                          ...
                          <td className="border-b border-[#eee] p-2">
                            <button
                              className="bg-primary text-white text-center p-2 rounded"
                              onClick={() => dispatch(showOrder(o.id))}
                            >
                              {tMyTicket("details")}
                            </button>
                          </td>
