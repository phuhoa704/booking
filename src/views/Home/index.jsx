...
const Home = () => {
  ...
  return (
    <>
      <section
        className="w-full bg-center bg-cover flex justify-center items-start xl:items-center relative h-smallContainer xl:h-largeContainer pt-16 xl:pt-0"
        style={{ backgroundImage: `url(${API_STORE}${banner})` }}
      >
          ...
          <div className="w-full bg-white rounded-lg shadow-secondary">
            <div className="flex justify-center py-3">
              <div className="flex pt-1.5 md:pt-0 xl:pt-3">
                ...
                >
                  <i className="fa-solid fa-bus"></i> {tHome("bus")}
                </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-4 h-full">
                  ...
                  <div className="col-span-2 xl:col-span-1 border-t border-r xl:border-t-0 border-[#d9d9d9] ">
                    {rendered && (
                      <DatePicker
                        ...
                        placeholderText={tHome("departure_date")}
                        selected={startDate}
                  ...
                  <div className="col-span-2 xl:col-span-1 border-t border-[#d9d9d9] xl:border-none">
                    {rendered && (
                      <DatePicker
                        className="px-2.5 py-2.5 bg-white border border-[#d9d9d9] w-full h-full rounded border-none focus:border-none"
                        placeholderText={tHome("return_date")}
                        selected={endDate}
        ...
        <div className="absolute bottom-0 py-2 xl:py-5 bg-blacktransparent w-full flex justify-center gap-4 xl:gap-12">
          <div className="flex items-center flex-col gap-2 xl:flex-row">
            ...
            <span
              className="font-semibold text-center text-xs xl:text-base"
              style={{ color: text ? text : "#fff" }}
            >
              {tHome("guaranteed_seat")}
            </span>
          </div>
          <div className="flex items-center gap-2 flex-col xl:flex-row">
            ...
            >
              <g id="Rounded / headset_mic">
                <path
                  id="Vector"
                  d="M11.4 1.02048C6.62 1.33048 3 5.52048 3 10.3105V17.0005C3 18.6605 4.34 20.0005 6 20.0005H7C8.1 20.0005 9 19.1005 9 18.0005V14.0005C9 12.9005 8.1 12.0005 7 12.0005H5V10.2905C5 6.45048 7.96 3.11048 11.79 3.00048C15.76 2.89048 19 6.06048 19 10.0005V12.0005H17C15.9 12.0005 15 12.9005 15 14.0005V18.0005C15 19.1005 15.9 20.0005 17 20.0005H19V21.0005H13C12.45 21.0005 12 21.4505 12 22.0005C12 22.5505 12.45 23.0005 13 23.0005H18C19.66 23.0005 21 21.6605 21 20.0005V10.0005C21 4.83048 16.64 0.680479 11.4 1.02048Z"
                  fill="#FFD333"
                />
              </g>
            </svg>
      ...
      <div className="w-full py-2">
        <div className="w-full xl:w-8/12 m-auto px-2 xl:px-0">
          <PopularRoad tPopularRoad={tHome} />
          {/* <PopularRoad />
                    <Concessionary />
          ...
          {listCategory.map((c) => (
            <section className="my-4" key={c.id}>
              <p className="font-semibold text-base xl:text-2xl text-[#3d3d3b] py-2.5">
                {localizedFields(c, i18n.language).name}
              </p>
              <div className="w-full overflow-x-auto scroll-horizontal pb-2">
                <div className="flex gap-4 w-max">
                  {c.news.map((d) => (
                    <div
                      className="w-[300px] flex flex-col bg-white shadow-xl border border-[#d9d9d9] rounded cursor-pointer"
                      key={`${c.id}_${d.id}`}
                      onClick={() => {
                    ...
                    >
                      <img
                        src={`${API_STORE}${d.banner}`}
                        className="w-full h-36 rounded-tl rounded-tr"
                        alt=""
                      />
                      <div className="p-2.5 rounded-bl rounded-br">
                        <p className="font-semibold">
                          {localizedFields(d, i18n.language).name}
                        </p>
                      </div>
                    </div>
                  ))}
          ...
          <Flatforms tFlatforms={tHome} />
          {/* <SocialMedia /> */}
          <BusStations tBusStations={tHome} />
        </div>
      </div>
      <Footer />
    </>
